...
| test\_checkInCheckOut | The time elapsed for check-in and check-out operations, ran in a POST request, is measured, to comply with QR-PF-02 and QR-PF-03. | The user tracing app and the cloud databases are up and running. | timeIn \<= 3 and timeOut \<= 10 and response\_in.status\_code == 200 and response\_out.status\_code == 200 | Pass |
